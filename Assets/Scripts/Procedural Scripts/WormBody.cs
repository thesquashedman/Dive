...
public class WormBody : MonoBehaviour
{
    ...
    // The position at which this worm should be fixed.
    ...
    void Start()
    {
        ...
        rigidbodies[0].transform.position = habitat.transform.position;
        for (int i = 1; i < length - 1; i++)
        {
            rigidbodies[i] = Instantiate(rigidbodyPrefab);
            rigidbodies[i].transform.position = habitat.transform.position;
        }
        rigidbodies[length - 1] = new GameObject();
        rigidbodies[length - 1].transform.position = habitat.transform.position;
    }
    ...
    void FixedUpdate()
    {
        ...
        jointPositions[length - 1] = habitat.transform.position;
        rigidbodies[length - 1].transform.position = jointPositions[length - 1];
