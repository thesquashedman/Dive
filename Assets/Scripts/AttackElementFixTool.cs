...
public class AttackElementFixTool : AttackElement
{
    ...
    public float fixAmount = 1;

    // Optionally, create a constructor for the subclass
    public AttackElementFixTool(AttackSystem attackSystem, float attackRange) : base(attackSystem)
    {
    ...
    private void OnTriggerStay2D(Collider2D other)
    {
        if (other.gameObject.CompareTag(tragetTag))
        {
            ...
            if (attackPeriodTimer > attackPeriod)
            {
                ...
                //attackPeriodTimer = 0;
                other.gameObject.GetComponent<FixObject>().Fix(fixAmount);
            }
